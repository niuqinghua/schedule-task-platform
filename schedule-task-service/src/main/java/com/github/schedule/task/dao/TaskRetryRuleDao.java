...
public interface TaskRetryRuleDao {
    ...
    @Insert("insert into task_retry_rule (task_type, retry_limit, status, created_time, updated_time) "
            + "values(#{taskType}, #{retryLimit}, #{status}, NOW(), NOW())")
    void add(TaskRetryRule taskRetryRule);

    @Update("update task_retry_rule set task_type = #{taskType}, retry_limit = #{retryLimit}, "
    ...
    @Select("select id, task_type as taskType, retry_limit as retryLimit, status, created_time as createdTime, updated_time as updatedTime "
